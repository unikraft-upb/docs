...
Session support files are available [in this repository](https://github.com/cristian-vijelie/guides-exercises).
...
```console
$ git clone https://github.com/cristian-vijelie/guides-exercises
...
First make sure you have the following conventional working directory also shown in [TODO](TODO).
...
You can check the source code for this tutorial in `04-tracepoints`.
...
Modify your `Echo-back Server` application implemented in the [first](TODO) guide, so that each time the server responds with a message, a tracepoint with the corresponding message will be activated.
...
The file is `bad_elf`, located in the `07-bad-elf` folder.
